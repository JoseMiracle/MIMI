...
class UserCancelOrRejectFriendRequestAPIView(generics.DestroyAPIView):
    ...
    def perform_destroy(self, instance):
        if instance is None:
            return Response(
                data={"status": "error"}, status=status.HTTP_204_NO_CONTENT
            )
...
class AcceptFriendRequestAPIView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"message": "friendship accepted already"},
              status=status.HTTP_200_OK
            )
...
class UserFriendsAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        blocked_user_ids = UsersBlockedByUser.objects.filter(user=user).values_list('other_user_id', flat=True)
        user_friends_queryset  = Friends.objects.filter(
                                                user=user).exclude(
