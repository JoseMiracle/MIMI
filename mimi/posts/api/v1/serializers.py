...
class ReactionToPostSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        post_reaction_obj, created = PostReaction.objects.get_or_create(user_post=validated_data["user_post"], user_that_react = self.context["request"].user,
                                                                defaults={
...
class PostSerializer(serializers.ModelSerializer):
    ...
    post_images = serializers.ListField(
        child=serializers.ImageField(required=False, allow_empty_file= True), required = False
    )
...
class UpdatePostSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if "post_state" in validated_data:
            instance.created_at, instance.last_modified_at = timezone.now(), timezone.now()
            instance.save()
...
class CommentToPostSerializer(serializers.ModelSerializer):
    ...
    @transaction.atomic
    def create(self, validated_data):
        uploaded_comment_to_post_images = validated_data.pop('uploaded_comment_to_post_images', None)
