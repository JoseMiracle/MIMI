...
from mimi.chats.utils.constants import PENDING_ROOM_REQUEST, ACCEPTED_ROOM_REQUEST
...
class JoinRoomRequests(BaseModel):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="user_room_request"
    )
    room_request = models.CharField(
        max_length=20, default=ROOM_REQUEST_CHOICES[0][0], choices=ROOM_REQUEST_CHOICES
    )
...
class RoomMembers(BaseModel):
    ...
    room_member = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="current_room_member"
    )
