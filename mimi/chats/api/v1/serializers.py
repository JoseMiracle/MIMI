from mimi.chats.models import Message, Room, RoomMembers, JoinRoomRequests
...
class RoomSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Room
        fields = ["id", "room_name", "description", "number_of_persons"]
...
class JoinRoomRequestSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["room_name", "room", "room_request"]

    def validate(self, attrs):
        is_room_request_exists = JoinRoomRequests.objects.filter(
            room=attrs["room"], user=self.context["request"].user
        ).exists()
