...
class UserRoomsAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        user_accepted_rooms_queryset = JoinRoomRequests.objects.filter(user=self.request.user, room_request=ACCEPTED_ROOM_REQUEST)
...
class GetAllUsersInTheRoomAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        room = Room.objects.filter(room_name=self.kwargs['room_name']).first()
        user_in_room = RoomMembers.objects.filter(room=room, room_member=self.request.user).first()
...
class RemoveUserFromARoomAPIView(generics.GenericAPIView):
    ...
    def get_object(self):
        ...
        room_member_obj = RoomMembers.objects.filter(room=room, room_member=user).first()   
